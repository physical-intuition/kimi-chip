...
module tb_x2_y5;
    ...
    endtask
    initial begin
        writes = 0; act_rdata = 0; weight_rdata = 0;
        for (k = 0; k < 16; k = k + 1) begin
            act_mem[k] = 0; weight_mem[k] = 0;
            for (r = 0; r < 16; r = r + 1) begin av = ((k*3+r*5)%16)-8; act_mem[k][r*4 +: 4] = av[3:0]; end
            for (c = 0; c < 16; c = c + 1) begin wv = ((k*7+c*3)%16)-8; weight_mem[k][c*4 +: 4] = wv[3:0]; end
        end
        repeat (3) @(posedge clk); rst_n <= 1;
        run_case(7); run_case(0); run_case(13);
        $display("PASS: X2-Y5 CAP_MARGIN=21 boundary refinement, K=7/0/13, 768 outputs verified");
        $finish;
    end
