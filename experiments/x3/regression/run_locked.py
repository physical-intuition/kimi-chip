...
MANIFEST = pathlib.Path(__file__).with_name("manifest_y1.json")
...
def main():
    ...
    out = build / "tb_x3_y1"
    compile_cmd = [
        "iverilog", "-g2012", "-Wall", "-s", "tb_x3_y1", "-o", str(out),
        str(ROOT / "experiments/x1/rtl/x1_y5_kimi.v"),
        str(ROOT / "experiments/x3/rtl/x3_y1_kimi.v"),
        str(ROOT / "experiments/x3/tb/tb_x3_y1.v"),
    ]
    ...
    if sim.returncode != 0 or expected_line not in sim.stdout:
        ...
        return 1
    print("LOCK PASS: testbench hash and required 768-output transcript match")
    return 0
