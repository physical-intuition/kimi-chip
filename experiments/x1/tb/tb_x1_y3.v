...
module tb_x1_y3;
    ...
    task run_case;
        ...
        begin
            ...
            @(posedge clk);
            start <= 0;
            // Deliberately change k_dim after launch. Y3 must use its latched
            // dimension, proving the external input is off the active path.
            k_dim <= 12'hA55;
            wait(done);
            @(posedge clk);
            #1;
    ...
    initial begin
        ...
        run_case(0);
        run_case(13);
        $display("PASS: Y3 latched-k banked MAC, K=7/0/13, 768 outputs verified");
        $finish;
    end
