...
rtl=(root/'experiments/x4/rtl/x4_y4_kimi.v').read_text()
checks={
 ...
 'separate_fold_state': all(x in rtl for x in ('state == S_FOLD_LO', 'state == S_FOLD_HI', 'state == S_ACCUM')),
 'registered_two_slice_fold': all(x in rtl for x in ('fold_carry', 'wide_accum[gr][gc][11:0]', 'wide_accum[gr][gc][23:12]')) and bool(re.search(r'S_FOLD_LO[\s\S]*S_FOLD_HI', rtl)),
 'no_direct_24bit_fold': not bool(re.search(r'wide_accum\[gr\]\[gc\]\s*<=\s*wide_accum\[gr\]\[gc\]\s*\+.*chunk_accum', rtl)),
 'tail_flush': "consumed + 1'b1 == k_dim_q" in rtl and 'fold_finishes_run' in rtl and 'S_FOLD_HI' in rtl,
 ...
 'y4_max_cap_failure_is_actioned': 'wire57969/Z' in (root/'experiments/x4/DESIGN_Y5.md').read_text() and 'wire57973/Z' in (root/'experiments/x4/DESIGN_Y5.md').read_text(),
 'cap_margin_30': 'export CAP_MARGIN = 30' in (root/'experiments/x4/flow/config_y5.mk').read_text(),
}
checks['chunk_range_proof'] = 16*64 <= (2**11-1)
checks['wide_range_proof'] = 65535*64 <= (2**23-1)
report={'status':'pass' if all(checks.values()) else 'fail','checks':checks,'proof':{'chunk_max_magnitude':16*64,'signed_12_max':2**11-1,'wide_max_positive':65535*64,'signed_24_max':2**23-1},'evidence':{'registered_readout_split':'X4-Y2 final extracted path drain_col[0] to out_wdata[12], 18 combinational cells, experiments/x4/artifacts/y2/reports/6_finish.rpt', 'registered_two_slice_fold':'X4-Y3 final extracted path chunk_accum[57][1] to wide_accum[57][16], 21 combinational cells, experiments/x4/artifacts/y3/reports/6_finish.rpt'}}
out=root/'experiments/x4/build/lint_y5.json'; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,indent=2)+'\n')
