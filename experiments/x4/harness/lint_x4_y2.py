...
rtl=(root/'experiments/x4/rtl/x4_y2_kimi.v').read_text()
checks={
...
checks['request_decode_is_progress_independent'] = 'issued < k_dim_q' not in rtl and 'if (state == S_REQ)' in rtl
...
out=root/'experiments/x4/build/lint_y2.json'; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,indent=2)+'\n')
