...
LOG="$X4/build/orfs_y1.log"
MARKER="$X4/.y1_flow_rc"
...
rm -f "$MARKER"
mkdir -p "$X4/build"
docker rm -f x4_y1_orfs >/dev/null 2>&1 || true
docker run --rm --name x4_y1_orfs \
  -v /home/kit/OpenROAD-flow-scripts/flow:/OpenROAD-flow-scripts/flow \
  -v /home/kit/kimi-chip:/home/kit/kimi-chip \
  -w /OpenROAD-flow-scripts/flow \
  openroad/orfs:latest \
  bash -lc 'set +e
    make DESIGN_CONFIG=/home/kit/kimi-chip/experiments/x4/flow/config_y1.mk
    rc=$?
    artifact=/home/kit/kimi-chip/experiments/x4/artifacts/y1
    rm -rf "$artifact"
