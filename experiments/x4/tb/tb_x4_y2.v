...
module tb_x4_y2;
    ...
    task run_case;
        ...
        begin
            ...
            // Stimulus repeats every 16 addresses, so compute the full-range
            // oracle in 4096 operations rather than 16 million.
            for(k=0;k<16;k=k+1)
                for(r=0;r<16;r=r+1)
    ...
    initial begin
        ...
        abort_and_restart();
        run_case(65535);
        $display("PASS: X4-Y2 hierarchical 12-to-24 fold regression, tail flush, reset/restart, signed extrema, exactly-once, and K=65535 verified");
        $finish;
    end
