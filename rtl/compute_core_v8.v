module compute_core_v8 (
    input wire clk,
    input wire rst_n,
    input wire start,
    input wire [1:0] k_dim,  // Max 4 for 8-bit acc
    output reg wgt_cs,
    output reg [1:0] wgt_addr,
    input wire [63:0] wgt_rdata,
    output reg act_cs,
    output reg [1:0] act_addr,
    input wire [63:0] act_rdata,
    output wire [256*8-1:0] acc_out,
    output reg done
);
    ...
    mac_array_v8 #(.ROWS(16), .COLS(16)) array (
        .clk(clk), .rst_n(rst_n),
