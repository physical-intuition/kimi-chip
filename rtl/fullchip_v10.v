// fullchip_v10 - the "full chip" answer to core-only fmax numbers.
//
// compute_core_v10 (streaming, RD_LAT=3) + two 4096x64 SRAMs built from the
// Nangate45 platform's own fakeram45_512x64 hard macros (8 per memory, 16
// total; 64KB) + host write port + narrow result readout. No wide buses
// reach the pins: the 256x24 accumulator bus stays internal.
//
// The fakeram45 macros are single-port RW: the host write phase (load) and
// the core read phase (compute) never overlap, so a phase mux on the port
// is architecturally clean.
//
// Memory read pipeline (per element):
//   T   : core registers addr
//   T+1 : fakeram registers the read (rd_out valid next edge), bank selects register
//   T+2 : 8:1 bank mux -> interface register
//   T+3 : MACs consume (RD_LAT=3)
module fullchip_v10 (
    input  wire        clk,
    input  wire        rst_n,
    ...
    compute_core_v10 #(.USE_CSA(0), .RD_LAT(3)) core (
