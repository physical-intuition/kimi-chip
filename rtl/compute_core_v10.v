...
module compute_core_v10 #(
    ...
    reg [2:0] state;
    reg [15:0] k_iss;
    reg [1:0] vpipe;        // issue-valid, delayed to match SRAM latency
    reg [3:0] addcnt;
    reg mac_clear, mac_drain;

    wire issuing = (state == RUN) && (k_iss != k_dim);
    wire mac_en  = vpipe[1];             // add exactly when rdata is valid
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                RUN: begin
                    ...
                    if (!issuing && vpipe == 2'b00) state <= FLUSH;
                end
