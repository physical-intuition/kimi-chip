// fullchip_os16 - systolic_os16 (output-stationary, zero-stall) + the
// v13 memory system. The core streams ONE word per operand per cycle
// continuously; the wrapper reads the pair containing element k and
// selects the group by k[0] (delayed to match the read pipeline).
// Host load port and readout identical to v13.
module fullchip_os16 (
    input  wire        clk,
    input  wire        rst_n,
    input  wire        h_we,
    input  wire        h_sel,
