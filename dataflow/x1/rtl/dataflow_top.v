`timescale 1ns/1ps
//-----------------------------------------------------------------------------
// X1: Complete KDA/MLA Dataflow Chip
...
module dataflow_top #(
    ...
    //-------------------------------------------------------------------------
    // Memory: KDA State SRAM (2 heads x 32x32 per layer)
    //-------------------------------------------------------------------------
    reg  [KDA_HEADS*KDA_DIM*KDA_DIM*32-1:0] kda_state [0:N_LAYERS-1];
    reg  [KDA_HEADS*KDA_DIM*32-1:0]         kda_q, kda_k, kda_v;
    reg  [KDA_HEADS*KDA_DIM*32-1:0]         kda_alpha, kda_beta;
    reg  [KDA_HEADS*KDA_DIM*32-1:0]         kda_out;

    //-------------------------------------------------------------------------
    // Memory: MLA KV Cache (grows with sequence)
    //-------------------------------------------------------------------------
    reg  [MLA_DC*32-1:0] mla_kv_cache [0:MAX_SEQ-1];
    reg  [7:0]           mla_kv_count;
    reg  [MLA_HEADS*MLA_DK*32-1:0] mla_q;
    reg  [MLA_DC*32-1:0]           mla_kv_new;
    reg  [MLA_HEADS*MLA_DV*32-1:0] mla_attn_out;

    //-------------------------------------------------------------------------
    // Conv History for alpha/beta gates
    //-------------------------------------------------------------------------
    reg  [CONV_KERNEL*KDA_HEADS*KDA_DIM*32-1:0] conv_k_history;
    reg  [CONV_KERNEL*KDA_HEADS*KDA_DIM*32-1:0] conv_v_history;

    //-------------------------------------------------------------------------
    // Compute: MAC Array (16x16 INT4)
    //-------------------------------------------------------------------------
    reg         mac_start;
    reg  [3:0]  mac_row, mac_col;
    reg  [3:0]  mac_a, mac_b;
    wire [23:0] mac_acc;
    wire        mac_done;
            ...
            case (current_phase)
                IDLE: begin
                    if (token_valid && token_ready) begin
                        busy_reg <= 1'b1;
                        current_layer <= 4'd0;
                        current_phase <= KDA_PROJ_Q;  // Start with layer 0 (KDA)
                        phase_cycle <= 8'd0;
                    end
                end
                ...
                MLA_KV_APPEND: begin
                    if (phase_done_7) begin
                        mla_kv_cache[mla_kv_count] <= mla_kv_new;
