`timescale 1ns/1ps
//-----------------------------------------------------------------------------
// X2Y5: FSM output registering - argmax precomputed at load, breaks case->data fanin @ 0.6ns: Complete KDA/MLA Dataflow Chip - MAC wired into KDA projections
...
module dataflow_top #(
    ...
    reg [D_MODEL*KDA_HEADS*KDA_DIM*4-1:0] weight_q_sr;
    reg [D_MODEL*KDA_HEADS*KDA_DIM*4-1:0] weight_k_sr;
    reg [D_MODEL*KDA_HEADS*KDA_DIM*4-1:0] weight_v_sr;
    wire [3:0] weight_q_head = weight_q_sr[3:0];
    wire [3:0] weight_k_head = weight_k_sr[3:0];
    wire [3:0] weight_v_head = weight_v_sr[3:0];
    ...
    reg  [23:0] mac_pipe;     // pipeline stage: MAC output captured before writeback
    reg         mac_pipe_v;   // pipeline valid
    ...
    integer wi;
    initial begin
        weight_q_sr = {(D_MODEL*KDA_HEADS*KDA_DIM*4){1'b0}};
        weight_k_sr = {(D_MODEL*KDA_HEADS*KDA_DIM*4){1'b0}};
        weight_v_sr = {(D_MODEL*KDA_HEADS*KDA_DIM*4){1'b0}};
        for (wi = 0; wi < D_MODEL*KDA_HEADS*KDA_DIM; wi = wi + 1) begin
            weight_q_sr[wi*4 +: 4] = wi[3:0] ^ 4'h5;
            weight_k_sr[wi*4 +: 4] = wi[3:0] ^ 4'hA;
            weight_v_sr[wi*4 +: 4] = wi[3:0] ^ 4'h3;
        end
    end
        ...
        if (!rst_n) begin
            ...
            case (current_phase)
                ...
                KDA_PROJ_Q: begin
                    ...
                    mac_b <= weight_q_head;
                    mac_pipe_v <= mac_done;
                    if (mac_done) mac_pipe <= mac_acc;
                    if (mac_pipe_v) begin
                        if (proj_mac_done) begin
                            kda_q[mac_out_idx*32 +: 32] <= {{8{mac_pipe[23]}}, mac_pipe};
                            mac_in_idx <= 8'd0;
                        ...
                        weight_q_sr <= {4'b0, weight_q_sr[D_MODEL*KDA_HEADS*KDA_DIM*4-1:4]};
                    end
                end
                ...
                KDA_PROJ_K: begin
                    ...
                    if (mac_pipe_v) begin
                        if (proj_mac_done) begin
                            kda_k[mac_out_idx*32 +: 32] <= {{8{mac_pipe[23]}}, mac_pipe};
                            mac_in_idx <= 8'd0;
                            if (proj_all_done) begin
                        ...
                        weight_k_sr <= {4'b0, weight_k_sr[D_MODEL*KDA_HEADS*KDA_DIM*4-1:4]};
                    end
                end
                ...
                KDA_PROJ_V: begin
                    ...
                    if (mac_pipe_v) begin
                        if (proj_mac_done) begin
                            kda_v[mac_out_idx*32 +: 32] <= {{8{mac_pipe[23]}}, mac_pipe};
                            mac_in_idx <= 8'd0;
                            if (proj_all_done) begin
                        ...
                        weight_v_sr <= {4'b0, weight_v_sr[D_MODEL*KDA_HEADS*KDA_DIM*4-1:4]};
                    end
                end
