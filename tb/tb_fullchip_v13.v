// tb_fullchip_v13 - lane-DISTINCT data (catches lane-slice bugs), plus the
// pairing hazards specific to v13: odd K (half-pair gating), K=1 (single
// half-pair), bank-group crossings (K=700 crosses row 512 in both groups,
// K=1030 crosses two banks), residual chunk, restart-after-DONE, and a
// throughput check (~K/2 + fill cycles).
`timescale 1ns/1ps
...
module tb_fullchip_v13;
  ...
  fullchip_v13 dut(.clk(clk), .rst_n(rst_n), .h_we(h_we), .h_sel(h_sel),
  ...
  initial begin
    ...
    run_k(1);    check(0,0,1); check(7,7,1);          // single half-pair
    run_k(2);    check(0,0,2);                         // one full pair
    run_k(37);   check(0,0,37); check(7,7,37); check(3,12,37);  // odd + residual chunk
    run_k(700);  $display("K=700 cycles=%0d (expect ~K/2+9=359)", cycles);
    check(0,0,700); check(7,7,700); check(15,15,700); check(3,12,700);
    run_k(1029); check(0,0,1029); check(7,7,1029);     // odd, two bank rows crossed
    run_k(1030); check(0,0,1030); check(15,15,1030); check(3,12,1030);
    run_k(700);  check(7,7,700);                       // restart after terminal DONE
