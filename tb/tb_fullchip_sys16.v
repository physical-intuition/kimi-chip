// tb_fullchip_sys16 - systolic core vs the same independent reference the
...
module tb_fullchip_sys16;
  ...
  fullchip_sys16 dut(.clk(clk), .rst_n(rst_n), .h_we(h_we), .h_sel(h_sel),
  ...
  initial begin
    ...
    run_k(700, 0);  $display("K=700 cycles=%0d (M1 stop-and-load; ~44 chunks x ~70)", cycles);
