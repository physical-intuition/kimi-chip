// tb_fullchip_os16p - systolic core vs the same independent reference the
...
module tb_fullchip_os16p;
  ...
  fullchip_os16p dut(.clk(clk), .rst_n(rst_n), .h_we(h_we), .h_sel(h_sel),
  ...
  initial begin
    ...
    run_k(700, 0);  $display("K=700 cycles=%0d (paired; expect ~K/2+48)", cycles);
